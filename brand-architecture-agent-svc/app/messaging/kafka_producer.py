"""Kafka producers for the Brand Architecture Agent.

Three producers:
- TraceProducer -> agent-trace-topic (real-time node progress)
- AuditProducer -> baa-architecture-audit-topic (audit trail)
- EventProducer -> baa-architecture-events-topic (architecture events)
"""
...
class TraceProducer:
    ...
    async def send_step(self, job_id: str, step: str, status: str = "running") -> None:
        await self.send_trace(job_id, status, message=f"BAA: {step}")


class AuditProducer:
    """Emits audit trail events to baa-architecture-audit-topic."""

    def __init__(self, bootstrap_servers: str) -> None:
        self._bootstrap = bootstrap_servers
        self._producer = None
        self._topic = "baa-architecture-audit-topic"
...
class EventProducer:
    """Emits architecture strategy events to baa-architecture-events-topic."""

    def __init__(self, bootstrap_servers: str) -> None:
        self._bootstrap = bootstrap_servers
        self._producer = None
        self._topic = "baa-architecture-events-topic"
