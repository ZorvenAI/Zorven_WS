"""Kafka producers for BPV service.

Three producers:
- TraceProducer → agent-trace-topic (real-time node progress)
- AuditProducer → bpv-personality-audit-topic (audit trail)
- EventProducer → bpv-personality-events-topic (personality events)
"""
...
class AuditProducer(_BaseProducer):
    """Produces audit trail to bpv-personality-audit-topic."""

    def __init__(self, bootstrap_servers: str):
        super().__init__(bootstrap_servers, "bpv-personality-audit-topic")
...
class EventProducer(_BaseProducer):
    """Produces personality events to bpv-personality-events-topic."""

    def __init__(self, bootstrap_servers: str):
        super().__init__(bootstrap_servers, "bpv-personality-events-topic")

    async def send_event(self, data: dict[str, Any]):
        """Send personality event."""
        await self.send(data)
