"""Async Redis manager for BPV service (DB 18, fail-open)."""
...
class RedisManager:
    ...
    async def get_cached_result(
        ...
        if not self._redis:
            return None
        try:
            key = f"bpv:{tenant_id}:result:{prompt_hash}"
    ...
    async def save_personality(
        ...
        """Save personality profile to registry (no TTL)."""
        if not self._redis:
            return
        try:
            key = f"bpv:{tenant_id}:registry:personality"
            await self._redis.set(key, json.dumps(personality, default=str))
        except Exception as exc:
            logger.warning("Redis save_personality error: %s", exc)

    async def get_personality(
        self, tenant_id: str
    ) -> dict[str, Any] | None:
        """Get personality profile from registry."""
        if not self._redis:
            return None
        try:
            key = f"bpv:{tenant_id}:registry:personality"
            ...
            logger.warning("Redis get_personality error: %s", exc)
            return None
