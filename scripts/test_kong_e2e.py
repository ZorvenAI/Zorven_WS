...
class TestConfig:
    ...
    verbose: bool = False
    timeout: int = 10
    test_user_email: str = field(default_factory=lambda: f"kong_test_{int(time.time())}@test.com")
...
def test_kong_proxy_reachable(config: TestConfig) -> tuple[bool, str, dict]:
    ...
    return False, f"Unexpected status: {response.status_code}", {"status_code": response.status_code}
...
def test_kong_admin_api(config: TestConfig) -> tuple[bool, str, dict]:
    ...
    if response.status_code == 200:
        data = response.json()
        return True, "Admin API responding", {
...
def test_kong_services_configured(config: TestConfig) -> tuple[bool, str, dict]:
    ...
    if response.status_code == 200:
        ...
        return False, f"Missing services. Found: {service_names}", {"services": service_names}
...
def test_kong_routes_configured(config: TestConfig) -> tuple[bool, str, dict]:
    ...
    if response.status_code == 200:
        ...
        expected = ["core-api-route", "auth-login-route", "auth-register-route", "health-routes"]
...
def test_kong_plugins_configured(config: TestConfig) -> tuple[bool, str, dict]:
    ...
    if response.status_code == 200:
        ...
        return False, f"Missing plugins. Found: {plugin_names}", {"plugins": plugin_names}
...
def test_register_endpoint_via_kong(config: TestConfig) -> tuple[bool, str, dict]:
    ...
    if response.status_code == 201:
        ...
        return True, "Registration endpoint working (user exists)", {"status": response.status_code}
    return False, f"Registration failed: {response.status_code}", {"response": response.text[:200]}
...
def test_login_endpoint_via_kong(config: TestConfig) -> tuple[bool, str, dict]:
    ...
    if reg_response.status_code != 201:
        return False, f"Setup failed - couldn't register: {reg_response.status_code}", {}
    ...
    return False, f"Login failed: {response.status_code}", {"response": response.text[:200]}
...
def test_protected_route_without_token(config: TestConfig) -> tuple[bool, str, dict]:
    ...
    return False, f"Expected 401, got {response.status_code}", {"status": response.status_code}


@run_test
def test_protected_route_with_invalid_token(config: TestConfig) -> tuple[bool, str, dict]:
...
def test_protected_route_with_valid_token(config: TestConfig) -> tuple[bool, str, dict]:
    ...
    if response.status_code in [200, 404]:  # 200 = has companies, 404 = no companies yet
        return True, "Authenticated request allowed", {"status": response.status_code}
    return False, f"Expected 200/404, got {response.status_code}", {"status": response.status_code}
...
def test_cors_actual_request(config: TestConfig) -> tuple[bool, str, dict]:
    ...
    response = requests.get(
        f"{config.kong_proxy_url}/health/",
        headers=headers,
        timeout=config.timeout
    )
...
def test_rate_limit_headers_present(config: TestConfig) -> tuple[bool, str, dict]:
    ...
    rate_limit = response.headers.get("X-RateLimit-Limit-Minute") or response.headers.get("RateLimit-Limit")
    rate_remaining = response.headers.get("X-RateLimit-Remaining-Minute") or response.headers.get("RateLimit-Remaining")
...
def test_celery_tasks_discovered(config: TestConfig) -> tuple[bool, str, dict]:
    ...
    try:
        ...
        if len(found_tasks) >= 2:
            return True, f"Found {len(found_tasks)} automation tasks", {"tasks": found_tasks}
...
def test_full_auth_flow_via_kong(config: TestConfig) -> tuple[bool, str, dict]:
    ...
    if final_response.status_code in [200, 404]:
        return True, "Full auth flow completed successfully", {
            "steps_completed": ["register", "login", "protected_access", "refresh", "new_token_access"]
        }
...
def test_kong_proxies_to_backend(config: TestConfig) -> tuple[bool, str, dict]:
    ...
    kong_response = requests.get(f"{config.kong_proxy_url}/health/", timeout=config.timeout)
...
def main():
    ...
    parser.add_argument("--kong-url", default="http://localhost:8000", help="Kong proxy URL")
    parser.add_argument("--kong-admin", default="http://localhost:8002", help="Kong admin URL")
    parser.add_argument("--mcp-url", default="http://localhost:8003", help="MCP server URL")
