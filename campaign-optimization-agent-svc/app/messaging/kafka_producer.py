"""Kafka producers for Campaign Optimization Agent.

Three producers:
- TraceProducer → agent-trace-topic (real-time node progress)
- AuditProducer → coa-optimization-audit-topic (audit trail)
- EventProducer → coa-optimization-events-topic (optimization events)
"""
...
class _BaseProducer:
    ...
    async def start(self):
        ...
        try:
            ...
            logger.warning(
                "Kafka producer start failed for %s: %s", self._topic, exc
            )
            self._producer = None
...
class AuditProducer(_BaseProducer):
    """Produces audit trail to coa-optimization-audit-topic."""

    def __init__(self, bootstrap_servers: str):
        super().__init__(bootstrap_servers, "coa-optimization-audit-topic")
...
class EventProducer(_BaseProducer):
    """Produces optimization events to coa-optimization-events-topic."""

    def __init__(self, bootstrap_servers: str):
        super().__init__(bootstrap_servers, "coa-optimization-events-topic")
