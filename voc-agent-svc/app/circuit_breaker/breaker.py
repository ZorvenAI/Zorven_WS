"""Circuit breaker implementation for the Voice of Customer Agent.

9 circuit breakers:
- tavily (5 failures, 30s recovery, fallback=CACHE)
- httpx (3 failures, 30s recovery, fallback=SKIP)
- odoo_helpdesk (3 failures, 60s recovery, fallback=SKIP)
- odoo_survey (3 failures, 60s recovery, fallback=SKIP)
- odoo_chatter (3 failures, 60s recovery, fallback=SKIP)
...
class CircuitOpenError(Exception):
    ...
    def __init__(self, name: str, fallback: FallbackAction) -> None:
        ...
        self.fallback = fallback
        super().__init__(
            f"Circuit '{name}' is open (fallback={fallback.value})"
        )


def create_breakers(settings: Any) -> dict[str, CircuitBreaker]:
    """Create all 9 circuit breakers from settings."""
    return {
