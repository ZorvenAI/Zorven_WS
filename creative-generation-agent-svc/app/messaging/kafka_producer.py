"""Kafka producers for CGA service.

Three producers:
- TraceProducer → agent-trace-topic (real-time node progress)
- AuditProducer → cga-creative-audit-topic (audit trail)
- EventProducer → cga-creative-events-topic (creative events)
"""
...
class AuditProducer(_BaseProducer):
    """Produces audit trail to cga-creative-audit-topic."""

    def __init__(self, bootstrap_servers: str):
        super().__init__(bootstrap_servers, "cga-creative-audit-topic")
...
class EventProducer(_BaseProducer):
    """Produces creative events to cga-creative-events-topic."""

    def __init__(self, bootstrap_servers: str):
        super().__init__(bootstrap_servers, "cga-creative-events-topic")
