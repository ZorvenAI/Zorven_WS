"""Async Redis manager for CGA service (DB 22, fail-open)."""
...
class RedisManager:
    ...
    async def get_cached_result(
        ...
        if not self._redis:
            return None
        try:
            key = f"cga:{tenant_id}:result:{prompt_hash}"
    ...
    async def check_rate_limit(self, tenant_id: str) -> bool:
        """Check if tenant is within rate limits (5/min for CGA due to image gen cost)."""
        if not self._redis:
            return True
        try:
            key = f"cga:rate:{tenant_id}"
    ...
    async def save_creative(self, tenant_id: str, creative_data: dict[str, Any]):
        """Save creative results to registry (no TTL)."""
        if not self._redis:
            return
        try:
            key = f"cga:{tenant_id}:registry:creative"
            await self._redis.set(key, json.dumps(creative_data, default=str))
        except Exception as exc:
            logger.warning("Redis save_creative error: %s", exc)

    async def get_creative(self, tenant_id: str) -> dict[str, Any] | None:
        """Get creative results from registry."""
        if not self._redis:
            return None
        try:
            key = f"cga:{tenant_id}:registry:creative"
            ...
            logger.warning("Redis get_creative error: %s", exc)
            return None
