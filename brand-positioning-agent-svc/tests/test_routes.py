...
class TestHealthEndpoint:
    ...
    async def test_health_returns_ok(self, client):
        ...
        assert data["service"] == "brand-positioning-agent"
...
class TestPositionEndpoint:
    """Test /v1/position alias endpoint."""

    async def test_position_requires_auth(self, client, valid_execute_payload):
        resp = await client.post("/v1/position", json=valid_execute_payload)
        assert resp.status_code == 422

    async def test_position_accepts_valid_token(
        self, client, valid_execute_payload, service_token_headers
    ):
        resp = await client.post(
            "/v1/position",
