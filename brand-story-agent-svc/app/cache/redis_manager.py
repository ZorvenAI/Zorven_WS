"""Async Redis manager for BSA service (DB 20, fail-open)."""
...
class RedisManager:
    ...
    async def get_cached_result(
        ...
        if not self._redis:
            return None
        try:
            key = f"bsa:{tenant_id}:result:{prompt_hash}"
    ...
    async def save_story(self, tenant_id: str, story_data: dict[str, Any]):
        ...
        if not self._redis:
            return
        try:
            key = f"bsa:{tenant_id}:registry:story"
            await self._redis.set(key, json.dumps(story_data, default=str))
        except Exception as exc:
    ...
    async def get_story(self, tenant_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.warning("Redis get_story error: %s", exc)
            return None
