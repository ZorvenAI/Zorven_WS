"""Kafka producers for BSA service.

Three producers:
- TraceProducer → agent-trace-topic (real-time node progress)
- AuditProducer → bsa-story-audit-topic (audit trail)
- EventProducer → bsa-story-events-topic (story events)
"""
...
class AuditProducer(_BaseProducer):
    """Produces audit trail to bsa-story-audit-topic."""

    def __init__(self, bootstrap_servers: str):
        super().__init__(bootstrap_servers, "bsa-story-audit-topic")
...
class EventProducer(_BaseProducer):
    """Produces story events to bsa-story-events-topic."""

    def __init__(self, bootstrap_servers: str):
        super().__init__(bootstrap_servers, "bsa-story-events-topic")
