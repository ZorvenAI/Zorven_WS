...
class TestHealthEndpoint:
    ...
    async def test_health_returns_ok(self, client):
        ...
        assert data["service"] == "brand-naming-agent"
...
class TestExecuteEndpoint:
    ...
    async def test_execute_stub_response_when_no_executor(
        ...
        assert data["name_candidates"] == []
        assert data["taglines"] == []
...
class TestNamingEndpoint:
    """Test /v1/naming alias endpoint."""

    async def test_naming_requires_auth(self, client, valid_execute_payload):
        resp = await client.post("/v1/naming", json=valid_execute_payload)
        assert resp.status_code == 422

    async def test_naming_accepts_valid_token(
        self, client, valid_execute_payload, service_token_headers
    ):
        resp = await client.post(
            "/v1/naming",
