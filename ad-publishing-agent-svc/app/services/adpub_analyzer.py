...
class AdPubAnalyzer:
    ...
    async def analyze_and_prepare(
        ...
        for i, stage in enumerate(bp.get("funnel_stages", [])):
            budget_pct = stage.get("budget_pct", 1.0 / max(
                len(bp.get("funnel_stages", [])), 1
            ))
            daily_budget_usd = (
                bp["total_budget_usd"]
                * budget_pct
            ...
            if i < len(targeting_results):
                targeting_spec = targeting_results[i].get(
                    "targeting_spec", {}
                )
            ...
            matching_packages = [
                p for p in packages
                if p.get("ad_set_name", "").startswith(
                    stage.get("stage", "")
                )
            ] or packages[:1]  # Fallback to first package
        ...
        for creative in creatives:
            preview_html = await self._meta.get_ad_preview(
    ...
    async def _create_campaign(
        ...
        try:
            campaign_id = await self._meta.create_campaign(
                ...
                special_ad_categories=blueprint.get(
            ...
            if trip and trip["action"] == BreakerAction.ESCALATE.value:
                logger.critical(
                    "Campaign creation circuit breaker tripped: %s", exc
                )
            raise
    ...
    async def _upload_image(
        ...
        try:
            ...
            if trip and trip["action"] == BreakerAction.RETRY.value:
                logger.warning(
                    "Image upload breaker tripped — retry allowed: %s", exc
                )
            raise
