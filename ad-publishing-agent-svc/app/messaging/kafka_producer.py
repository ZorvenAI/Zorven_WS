"""Kafka producers for Ad Publishing Agent.

Three producers:
- TraceProducer → agent-trace-topic (real-time node progress)
- AuditProducer → apa33-publishing-audit-topic (audit trail)
- EventProducer → apa33-publishing-events-topic (publishing events)
"""
...
class AuditProducer(_BaseProducer):
    """Produces audit trail to apa33-publishing-audit-topic."""

    def __init__(self, bootstrap_servers: str):
        super().__init__(bootstrap_servers, "apa33-publishing-audit-topic")
...
class EventProducer(_BaseProducer):
    """Produces publishing events to apa33-publishing-events-topic."""

    def __init__(self, bootstrap_servers: str):
        super().__init__(bootstrap_servers, "apa33-publishing-events-topic")
